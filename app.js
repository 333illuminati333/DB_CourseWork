...
setInterval(dbUpdate.getCitiesInfo, 60000);

app.get('/backup', function(req, res)
{
    const pathToSave = path.join(__dirname, `./backup`);
    const command = `cd C:\\Program Files\\MongoDB\\Server\\4.2\\bin & mongodump --host localhost --port 27017 --forceTableScan --db mydb --out ${pathToSave} `;
    child_process.exec(command);
    res.redirect('/?backup=successful');
});
...
app.get('/restore', function(req, res)
{
    ...
    .then(() =>
    {
        ...
        const command = `cd C:\\Program Files\\MongoDB\\Server\\4.2\\bin & mongorestore ${pathToRestore} `;
        console.log(child_process.exec(command));
        res.redirect('/?restore=successful');
    });
});
